...
- Повторный вход: сначала проверь memory(action="list") и дочитай недостающие ключи через memory(action="get", ...), затем добери только отсутствующие данные.
...
- Перед transition обязательно проверь их наличие через memory(action="list") и, при необходимости, memory(action="get", key="...").
